...
function StateQualifiersHome() {
  ...
  const createFilterOptions = useCallback(() => {
    ...
    return uniqueSchools
      .filter(racer => racer.school && racer.school.length > 0)
      .map(racer => {
        return {
  ...
  const findKidsWithThreeRaces = useCallback(
    csvData => {
  ...
  // tableColumns in state vs mutating these columns
  useEffect(() => {
    const columns = [
      {
        title: "First Name",
    ...
    setColumns(columns);
  }, [qualifiers, createFilterOptions]);

  useEffect(() => {
    const calculatedQualifiers = findKidsWithThreeRaces(allRacers);
    setQualifiers(calculatedQualifiers);
  }, [numberOfRacesToQualify, allRacers, findKidsWithThreeRaces]);
